...
\item If the merge program exits with a non-zero status, then merge is
  ...
  \textbf{unless} the command created two files that are actually
  equal, or modified the two source files in equal ways.
...
\item If the files \verb|NEW1|, \verb|NEW2|, and \verb|NEWARCH| have all
  ...
  the backup directory to reflect the fact that the file is currently
  in sync. 
\item If \verb|NEW1| and \verb|NEW2| (resp. \verb|CURRENT1| and
  \verb|CURRENT2|) are created (resp. overwritten) with different
  contents, but the merge command did not fail -- exited with 0, then
  we arbitrarily propagate \verb|NEW1| (resp. \verb|CURRENT1|) to the
  other replica and to the archive. This is a design choice made to
  handle the case where a merge command only synchronizes some
  specific contents between two files, skipping some irrelevant
  information (order between entries, for instance). We assume that
  only not important information is trimmed during the merge
  process. If some important information is ignored by the merge
  command, then it will eventually be lost on the distant replica,
