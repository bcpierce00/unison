...
See \ONEURL{https://github.com/bcpierce00/unison/wiki/Downloading-Unison}.
...
If version interoperability requirements are followed then Unison 2.52 can
upgrade the archive created by earlier Unison versions.  To avoid rebuilding
archive files, you must install version 2.52 built with the same OCaml
version as your previous version of Unison, and then run it at least once
on each root.  Doing so will upgrade the archive file.

After upgrading the archive, you are free to swap the Unison 2.52
...
{\bf Unison 2.52} is compatible with:
\begin{itemize}
...
NB: Unison has not been designed to run with elevated privileges, and
it has not been audited for that environment.
Therefore Unison should be run with the userid of the owner of the
files to be synchronized.
Note also that some security schemes permit granting privileges to
bind to low-number ports while at the same time prohibiting reading
environment variables.
Problems from this are not bugs in Unison but a consequence of using
Unison outside its design space.
...
socket's parent directory and the {\tt umask} value when starting
Unison.
...
\end{alltt}
(socket path is enclosed in curly braces).  Note that Unix domain
sockets are local sockets (they exist in the filesystem namespace).
To connect over a Unix socket remotely, you have to forward the socket
by other means, for example by using {\tt spiped} secure pipe daemon.
