...
The preference \verb|merge| controls how this program is invoked.  
...
Many merge commands, like \verb|diff3| for instance, require not only
the two files to be merged, but also a file containing the {\em last
synchronized version} of a file. You can ask Unison to keep
such specific archives for some files for you by using the
\verb|backupcurrent| preference. This preference is used in exactly
the same way as \verb|backup|. When adding a path specification to
\verb|backupcurrent|, Unison will keep a backup of the current
synchronized version of all files matching this path specification,
and update it every time the files are updated in one or both replicas.
These backups are kept on {\em both} replicas in the same place as
regular backups are, i.e. according to the \verb|backuplocation| and
\verb|backupdir| preferences. They are named like the original files
if \verb|backupslocation| is set to 'central' and otherwise, Unison
uses the \verb|backupprefix| and \verb|backupsuffix| preferences and
assumes a version number 000 for these backups.
