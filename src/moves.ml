...
let () = featMovesValid :=
  fun feats enabledThis ->
    if Prefs.read detectmoves
        && not (enabledThis && Safelist.mem "prevState2" feats) then
...
let extractStamp rc =
  ...
  | Updates (uc, prev) ->
      ...
      | Absent | File (_, ContentsSame) ->
          ...
          | ArchiveFile (_, _, stamp, _) -> stamp
          | ArchiveDir _ | ArchiveSymlink _ | NoArchive -> assert false
          end
...
let extractProps rc =
  ...
  | Updates (uc, prev) ->
      ...
      | Absent ->
          ...
          | ArchiveDir (props, _) -> props
          | ArchiveSymlink _ | NoArchive -> assert false
          end
      | File (props, _)
...
let processHint rep path differ = function
  ...
  | {status = `Deleted; ui = Updates (Absent, ArchiveFile (_, fp, _, _)); _} ->
      (* Deleted file -- potential move source *)
      addFileCandidate false rep path fp differ
  | {status = `Created; ui = Updates (Dir _, _) as ui; _} ->
  ...
  | {status = `Deleted; ui = Updates (Absent, (ArchiveDir _ as arch)); _} ->
      (* Deleted directory -- potential move source *)
      let archiveHash = hashArchive arch 0 in
      addDirCandidate false rep path archiveHash differ
  (* Not interested in the following *)
  | {status = `Created; ui = Updates (Symlink _, _); _}
  | {status = `Deleted; ui = Updates (_, ArchiveSymlink _); _}
  ...
  | {status = `Deleted; ui = Updates (_, NoArchive); _} ->
      ()
