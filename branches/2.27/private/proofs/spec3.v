...
Lemma child_same_kind :
  (k : Kind) (c : Name -> FS) (n : Name)
  (child k (Node k c) n) = (c n).
Intros k c n; Unfold child; Case (eq_dec_kind k k);
  [ Trivial | Intro E; Case E; Trivial ].
Qed.
...
Lemma one_fs_rec :
  (f : FS -> Set)
  ((a : FS)
   ((k : Kind) (n : Name)(f (child k a n))) -> (f a)) ->
  (f Bot) ->
  (a : FS) (f a).
Intros f H H' a; Apply FS_rec;
  [ Intros k c H''; Apply H;
    Simpl; Intros k' n; Case (eq_dec_kind k' k); Trivial
  | Trivial ].
Qed.
...
Lemma three_fs_rec :
  ...
  (o,a,b : FS)
  (f o a b).
Intros P H H' o; Generalize H H'; Clear H' H; Pattern o; Apply one_fs_rec;
  [ Auto
  | Intros H H' a; Generalize H H'; Clear H' H; Pattern a; Apply one_fs_rec;
      [ Auto
      | Intros H H' b; Pattern b; Apply one_fs_rec; Auto ] ].
Qed.
...
Axiom eq_fs_dec : (a, b : FS) {a = b} + {~ a = b}.
...
Intros o a b; Pattern o a b; Apply three_fs_rec;
  [ Clear o a b; Intros o a b;
    Cut { k : Kind &
        { ca : Name -> FS &
        { cb : Name -> FS |
          a = (Node k ca) /\ b = (Node k cb)}}}
      + {a = Bot \/ b = Bot \/
         (EX ka : Kind |
         (EX kb : Kind |
         (EX ca : Name -> FS |
         (EX cb : Name -> FS |
          a = (Node ka ca) /\ b = (Node kb cb) /\ ~ ka = kb))))};
      [ Intro H; Case H; Clear H;
          [ Intro H; Case H; Clear H; Intro k;
            Intro H; Case H; Clear H; Intro ca;
            Intro H; Case H; Clear H; Intro cb;
            Intros (E1, E2); Rewrite E1; Rewrite E2; Clear E1 E2;
            Intro H;
            Case (Choice2 Name FS
              [n, o']{a':FS &
                     {b':FS |
                      ((o'',a'',b'':FS)
                      (ok (child k o n) (child k (Node k ca) n)
                          (child k (Node k cb) n) o'' a'' b'') ->
                      (ok o'' a'' b'' o' a' b'))}});
              [ Trivial |
                Intros co' H1;
                Case (Choice2 Name FS
                        ...
                        Inversion H4;
                          [ Inversion H0;
                              [ Apply rec;
                                Simpl; Case (eq_dec_kind k k);
                                  [ Intros E' n; Apply H3;
                                    Rewrite <- (child_same_kind k ca);
                                    Rewrite <- (child_same_kind k cb);
                                    Rewrite H14; Apply atomic; Apply id
                                  | Intro E'; Case E'; Trivial ]
                              | Apply rec;
                                Simpl; Case (eq_dec_kind k k);
                                  [ Intros E' n; Apply H3;
                          ...
                          | Apply rec;
                            Simpl; Case (eq_dec_kind k k);
                              [ Intros E' n; Apply H3;
                                Rewrite <- (child_same_kind k co'0);
                                Rewrite <- (child_same_kind k ca'0);
                                Rewrite <- (child_same_kind k cb'0);
                                Apply H11
                              | Intro E'; Case E'; Trivial ] ] ] ] ]
          | Case (eq_fs_dec o a);
              [ Intros E E'; Rewrite <- E;
                Exists b; Exists b; Exists b;
                Intros o'' a'' b'';
                Pattern o a b o'' a'' b'';
                Apply six_fs_ind;
                  [ Clear H E E' o a b o'' a'' b'';
                    Intros o a b o' a' b' H' H'';
                    Inversion H'';
                      [ Inversion H; Apply atomic; Apply rtl
                      | Apply rec;
                  ...
                  | Trivial ]
              | Case (eq_fs_dec o b);
                  [ Intros E E' E''; Rewrite <- E;
                    Exists a; Exists a; Exists a;
                    Intros o'' a'' b'';
                    Pattern o a b o'' a'' b'';
                    Apply six_fs_ind;
                      [ Clear H E E' E'' o a b o'' a'' b'';
                        Intros o a b o' a' b' H' H'';
                        Inversion H'';
                          [ Inversion H; Apply atomic; Apply ltr
                          | Apply rec;
                            Rewrite H1; Rewrite H3; Rewrite H4; Rewrite H5;
                            Intro n; Apply H';
                            Rewrite <- H1; Rewrite <- H2; Rewrite <- H3;
                            Rewrite <- H4; Rewrite <- H5;
                            Auto ]
                      | Trivial ]
                  | Case (eq_fs_dec a b);
                      [ Intros E E1 E2 E3; Rewrite <- E;
                        Exists a; Exists a; Exists a;
                        Intros o'' a'' b'';
                        Pattern o a b o'' a'' b'';
                        Apply six_fs_ind;
                          [ Clear H E E1 E2 E3 o a b o'' a'' b'';
                            Intros o a b o' a' b' H' H'';
                            Inversion H'';
                              [ Inversion H; Apply atomic; Apply sync
                              | Apply rec;
                                ...
                                Auto ]
                          | Trivial ]
                      | Intros E1 E2 E3 E4 H;
                        Exists o; Exists a; Exists b;
                        Intros o'' a'' b'';
                        Intro H'; Generalize E1 E2 E3 E4;
                        Clear E1 E2 E3 E4;
                        Inversion H';
                          [ Inversion H0;
                              [ Intros; Apply atomic; Apply id
                              | Intros E1 E2 E3; Case E3; Trivial
                              | Intros E1 E2; Case E2; Trivial
                              | Intro E; Case E; Trivial ]
                          | Intros E1 E2 E3 [E4 | [E4 | E4]];
                              [ Discriminate
                              | Discriminate
                              | Case E4; Intro k';
                                Intro G; Case G; Clear G; Intro kb;
                                Intro G; Case G; Clear G; Intro ca';
                                Intro G; Case G; Clear G; Intro cb';
                                Intros (F1, (F2, F3));
                                Injection F1; Injection F2;
                                Intros F4 F5 F6 F7;
                                Rewrite <- F5 in F3;
                                Rewrite <- F7 in F3;
                                Case F3; Trivial ] ] ] ] ] ]
      | Case a;
          [ Intros ka ca; Case b;
              [ Intros kb cb;
                Case (eq_dec_kind ka kb);
                  [ Intro E; Rewrite E;
                    Left; Exists kb; Exists ca; Exists cb; Auto
                  | Intro E; Right; Right; Right;
                    Exists ka; Exists kb; Exists ca; Exists cb; Auto ]
              | Auto ]
          | Auto ] ]
  | Exists Bot; Exists Bot; Exists Bot;
    Intros o'' a'' b'' H;
    Inversion H; Inversion H0; Apply atomic; Apply id ].
Qed.

Require Extraction.

Write Caml File "spec.ml" [maximal_element].
